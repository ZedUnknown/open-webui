<script lang="ts">
	...
	import ImageGeneration from '$lib/components/common/animated/ImageGeneration.svelte';
	...
	export let selectedModels = [] as any;
...
{#key message.id}
	...
	>
		...
		<div class="flex-auto w-0 pl-1 relative">
			...
			<div>
				<div class="chat-{message.role} w-full min-w-full markdown-prose mt-1.5">
					<div>
						<!-- Show Event Emitters History -->
						{#if 
						...
						{:else}
							<!-- Show Image Generating Animation -->
							{#if 
								message.statusHistory?.some(status => status.action === 'image_generation')
								&& !message.done
							}
